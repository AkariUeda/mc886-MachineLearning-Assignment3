...
clustering = KMeans(n_clusters=clusters, n_init=10, max_iter=100000, n_jobs=-1).fit(train_set)
...
for c in range(number_of_clusters):
	...
	print("\nMedoids cluster {} (size={}): {}".format(c,len(clusters[c]),tweets.iloc[train_tweets[idx_clusters[c][0][medoid]]]['headline_text']))
	print("   {} nearest points:".format(len(n_neighbors)))
	for n in range(len(n_neighbors)):
		print("          {}".format(tweets.iloc[train_tweets[idx_clusters[c][0][n_neighbors[n]]]]['headline_text']))
	...
	for n in range(n_random):
		print("          {}".format(tweets.iloc[train_tweets[idx_clusters[c][0][random_idx[n]]]]['headline_text']))
